<template>
  <div class="ecard">
    <headerBar title="收货地址"></headerBar>
    <div class="container">
      <div class="address">
        <div class="addinfo" v-for="(address,ind) in addressList" :key="'address_'+ind">
          ...
          <div class="editor">
            <img src="./imgs/editor.png" alt @click="$router.push(`edit?id=${address.id}`)">
            <div class="select" @click="saveAddress(address, ind)" :class="{'active': activeSub==ind }"></div>
    ...
    <div class="add-btn" @click="$router.push('edit')">
      <b>+</b>新增
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    saveAddress (address, i) {
      if (this.$route.query.getAddressStr) {
        this.$router.push(
          '/' +
      ...
      this.activeSub = i;
      if (this.$route.query.onlyforId == 1) {
        let targetPath = this.$route.query.targetPath;
        let connector = targetPath.indexOf('?') > -1 ? '&' : '?';
        this.$router.push(
          '/' +
            this.$store.state.merchantAppid +
            '/merchant/' +
      ...
      MessageBox({
        message: `${this.$route.query.proName}将送往:${address.provinceName}${
          address.cityName
        ...
        if (action == 'confirm') {
          ...
          if (orderCode) {
            saveAddress(orderCode, address.id, rs => {
              Toast(rs.payload);
              setTimeout(() => {
                let targetPath = this.$route.query.targetPath;
                let connector = targetPath.indexOf('?') > -1 ? '&' : '?';
