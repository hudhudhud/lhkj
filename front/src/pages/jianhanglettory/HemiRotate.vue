...
export default {
  ...
  methods: {
    ...
    rotate (prize) {
      ...
      var angles =
        prize * (360 / this.setting.names.length) -
        360 / (this.setting.names.length * 2);
      if (angles < 270) {
        angles = 270 - angles;
      } else {
    ...
    async drawPannel () {
      ...
      this.ctx.font = Math.round(r / 11) + 'px blod Microsoft YaHei';
      let lineHeight = Math.round(r / 20);
      let textR = r - lineHeight * 8.5 - 20 * ratio;

      // 绘制奖品
      for (let i = 0; i < this.setting.names.length; i++) {
        var angle = this.setting.startAngle + i * arc;
        var sub = i % this.setting.colors.length;
        this.ctx.fillStyle = this.setting.colors[sub];
        this.ctx.beginPath();
        ...
        if (text.length === 4 && text.indexOf('元') === -1) {
          ...
          for (var j = 0; j < texts.length; j++) {
            this.ctx.fillText(
              ...
              -(60 + j * -lineHeight) * ratio,
              this.ctx.measureText(texts[j]).width,
              lineHeight
            );
          }
        } else if (text.indexOf('元') && text.length > 5) {
          // 奖品名称长度超过一定范围
        ...
        let w = 85 * 0.5;
        let h = 53 * 0.5;
        ctx.drawImage(img, -(w / 2) * ratio, -r * 0.26, w * ratio, h * ratio);
    ...
    redraw () {
      ...
      let borderWidth = contentWidth * 0.038;
      ...
      container.style.left = borderWidth * 1.01 + 'px';
      container.style.top = borderWidth * 1 + 'px';
