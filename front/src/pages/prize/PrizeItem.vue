<template>
  <div class="order-item">
    <div class="order-date-code">
      ...
      <!-- <div class="order-item-paynotice" v-if="order.payState==PAY_STATE_CREATE && canPay">请在{{latestPayTimeFormatted}}内完成支付</div> -->
    </div>
    ...
    <div class="order-item-info">
      <div class="order-item-info-content">
        <div class="order-item-name">
          <!--  {{order.proName}} -->
          <!-- <br/> -->{{order.proStandardName}} &times;{{order.commodityNum}}
          <!-- <div class="order-item-tag" v-if="order.credit>0">积分兑换</div> -->
        </div>
        <div v-if="showPrize" class="order-item-price">¥{{order.actualPrice}}</div>
      </div>
      <!-- 龙卡大转盘商户号 生产211  测试231    -->
      <!-- 佛山大转盘商户号    测试235 -->
      <div class="order-item-state"
        v-if="$store.state.merchantId==211 && order.proType==10 || $store.state.merchantId==235 && order.proType==10">
        <span v-if="order.fishState==FISH_STATE_CREATE">待受理</span>
        <span v-else-if="order.fishState==FISH_STATE_CALLBACK_SUCCESS">已发放</span>
        <span v-else-if="order.fishState==6">抽奖无效</span>
      </div>
      <div class="order-item-state" v-else>
        <!-- 未支付 -->
        ...
        <template v-if="order.payState==PAY_STATE_PAID">
          <!-- <div class="order-item-state-item">已支付</div> -->
          <div class="order-item-state-btn" v-if="order.fishState==FISH_STATE_CREATE">
            <span v-if="order.proType=='1'" @click="charge">立即充值</span>
            <span v-if="order.proType=='2'" @click="getCardPwd">查看卡密与规则</span>
            <span v-if="order.proType==PRODUCT_TYPE_COMMODITY" @click="getAddress">查看收货地址</span>
          </div>
          ...
          <template v-else-if="order.fishState==FISH_STATE_CALLBACK_SUCCESS">
            ...
            <div v-if="order.proType=='2'" class="order-item-state-btn" @click="getCardPwd">查看卡密与规则</div>
          ...
          <div v-else-if="order.proType=='2'" class="order-item-state-btn">
            <span @click="getCardPwd">查看卡密与规则</span>
          </div>
        </template>
