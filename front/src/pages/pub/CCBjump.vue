...
export default {
  methods: {
    ...
    jumpIOS (funcId, para) {
      this.executeCalliOS(
        'innerAppCall://CCBCallCenter?funcid=' + funcId + '&' + para
      );
    },
    ...
    jumptofunc (funcId, paraAndroid, paraIOS) {
      ...
      if (isChromeOrSansung || isAndroid) {
        ...
        this.jumpIOS(funcId, decodeURIComponent(paraIOS));
      }
    }
  },
  created () {
    // this.jumptofunc('06001005', 'business_param=01014', 'paymentInfo=01014')
    this.jumptofunc(this.$route.params.code, this.$route.query.parse, this.$route.query.parse);
    setTimeout(() => {
      this.$router.go(-1);
    }, 1000);
  }
