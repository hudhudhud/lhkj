<template>
  <div class="bg">
    <img src="./imgs/背景.png" alt width="100%">
    ...
    <div class="lottery">
      <div class="btn-group">
        ...
        ></button>
        <button class="lettery-rules-btn" @click="showRules(act.detail)"></button>
      </div>
      <HemiRotate
        ref="myRotate"
        v-if="showPannel"
      ...
      ></HemiRotate>
      <img class="btn-start" @click="start" src="./imgs/点击抽奖.png">
    </div>
  </div>
</template>

<script>
import pannelImage from './imgs/light.png';
import pointerImage from './imgs/go.png';
...
export default {
  ...
  methods: {
    ...
    startBullet () {
      let num = parseInt(this.bulletList.length * Math.random());

      let bullet = this.bulletList[num];
      let fly = {
        name: bullet.name,
