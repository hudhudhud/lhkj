<template>
  <div class="seckill">
    ...
    >
      <mt-swipe :auto="4000">
        <mt-swipe-item
          v-for="(img, ind0) in templateConfig.bannerList"
          :key="'img_' +ind0"
        >
          <img
            :src="img.url"
    ...
    <!--  <div class="electronic-board" v-for="(product,ind) in act.productList" :key="'board'+ind">
      <div class="notice-item" v-for="(std, index) in product.standardList" :key="'dateTime'+index">
        <div class="item">
          <div class="time">周{{dateTimeTxt[dateTime[index]-1]}}&nbsp;{{timeInterval[index]}}</div>
          <div class="state" v-if="thisDay!= dateTime[index]">未开始</div>
          <template v-else-if="timeStatusMap[std.id]">
            <div class="state arrow" v-if="timeStatusMap[std.id].isInTime">进行中</div>
            <div
              class="state arrow"
...
.seckill {
  ...
  .electronic-board {
    ...
    .notice-item {
      ...
      height: 1.02rem;
      .item {
        @include flex(column, normal, center, center);
        flex: 1;
        height: 1.02rem;
        .time {
          font-size: 0.26rem;
