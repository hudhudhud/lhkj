<template>
  <div>
    <cbbHeader
      ...
      :bgColor="templateConfig.color?templateConfig.color:'#f6253a'"
    ></cbbHeader>
    <div class="buy">
      <div class="tab">
      ...
      >
        ...
        >
          ...
          >
            <div class="tip" v-if="showStatus != 151"></div>
            <div class="currentPrice">{{std.currentPrice}}元</div>
            <div class="originalPrice" v-if="showStatus != 151">售价:{{(std.originalPrice * 0.93).toFixed(2)}}元</div>
            <div class="originalPrice" v-else>售价:{{std.originalPrice}}元</div>
      ...
      <div class="wrapper">
        ...
        <div
          class="btn"
          @click="createOrder"
          :style="{'background-color':templateConfig.color?templateConfig.color:'#f6253a'}"
        >确认</div>
        <div class="detail">
...
export default {
  ...
  data () {
    return {
      ...
      standard: {},
      productId: '',
      showStatus: process.env.PUBLISH_ENV == 'prod' ? this.$store.state.merchantId : this.$store.state.merchantId
    };
  },
  components: {
    cbbHeader
  ...
  methods: {
    ...
    async createOrder () {
      ...
      if (this.showStatus == 151) {
        this.gotoPay();
      } else {
        MessageBox({
          message: '仅支持银联信用卡支付，选用借记卡将支付失败。',
          title: '中行支付',
  ...
  mounted () {
    if (this.templateConfig.color) {
      this.$refs.mycolor.style.setProperty('--bcColor', this.templateConfig.color);
    }
  }
};
</script>
