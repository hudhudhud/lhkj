...
export default {
  ...
  methods: {
    ...
    async drawPannel () {
      ...
      this.ctx.strokeStyle = '#84b9f8';

      // font 属性设置或返回画布上文本内容的当前字体属性
      this.ctx.font = Math.round(r / 10) + 'px Microsoft YaHei';
      let lineHeight = Math.round(r / 8);
      let textR = r - lineHeight * 2 - 40 * ratio;

      // 绘制奖品
      for (let i = 0; i < this.setting.names.length; i++) {
        var angle = this.setting.startAngle + i * arc;
        let item = i % this.setting.colors.length;
        this.ctx.fillStyle = this.setting.colors[item];
        this.ctx.beginPath();
        ...
        if (text.length > 0) {
          ...
          if (text.indexOf('元') > 0) {
            text =
              text.substring(0, text.indexOf('元') + 1) +
              '||' +
              text.substring(text.indexOf('元') + 1, text.indexOf('元') + 4) +
              '||' +
              text.substring(text.indexOf('元') + 4, text.indexOf('元') + 7) +
              '||' +
              text.substring(text.indexOf('元') + 7);
          } else {
            text =
              text.substring(0, 4) +
              '||' +
        ...
        ctx.drawImage(img, -20 * ratio, -r * 0.45 - 10, 40 * ratio, 40 * ratio);
