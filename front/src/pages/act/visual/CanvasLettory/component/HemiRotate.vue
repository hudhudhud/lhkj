<template>
  <div class="hemi-rotate">
    <div class="award">
      <div class="award-bg">
        <img :src="templateConfig.circleBg" />
      </div>
      <div class="canvas-container"></div>
      <img @click="start" class="award-pointer" :style="{width:pointerWidth}" :src="templateConfig.pointerImage" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async drawPannel () {
      ...
      let textR = r - lineHeight * 1.5 - 25 * ratio;
      ...
      for (let i = 0; i < this.setting.names.length; i++) {
        ...
        let color =
          i % 2 == 0
            ? this.templateConfig.pannelColor1
            : this.templateConfig.pannelColor2;
        ...
        let w = img.width / 2.5;
        let h = img.height / 2.4;
        ctx.drawImage(img, -(w / 2) * ratio, -r * 0.35, w * ratio, h * ratio);
