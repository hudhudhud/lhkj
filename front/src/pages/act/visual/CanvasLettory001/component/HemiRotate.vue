  ...
  methods: {
    ...
    async drawPannel () {
      ...
      for (let i = 0; i < this.setting.names.length; i++) {
        ...
        // ----绘制奖品开始----
        if (i % 2 === 0) {
          this.ctx.fillStyle = '#075384'; // color1
        } else {
          this.ctx.fillStyle = '#075384'; // color2
        }
        var text = this.setting.names[i];

        // translate方法重新映射画布上的 (0,0) 位置
        this.ctx.translate(
          r + Math.cos(angle + arc / 2) * textR,
          r + Math.sin(angle + arc / 2) * textR
        );
        ...
        ctx.drawImage(img, -(w / 2) * ratio, -r * 0.3, w * ratio, h * ratio);
