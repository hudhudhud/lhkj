<template>
  <div class="hemi-grid">
    ...
    <!-- <div class="box"> -->
    <div class="box" :style="{'backgroundImage': 'url(' + templateConfig.pannel+')'}">
      <!-- <div class="border-light"> -->
      <div class="border-light" :style="{'backgroundImage': 'url('+ borderLightSrc+')'}">
        <div v-for="(imgLi, index) in prizeList" :key="'imgLi'+index" ref="pice" :id="imgLi.standardId" :class="{ 'active': index == arrNum[lottery.index]} " :style="{'backgroundImage':'url('+imgLi.bgSrc+')'}">
          <!-- <div v-for="(imgLi, index) in prizeList" :key="'imgLi'+index" ref="pice" :id="imgLi.standardId" :style="{'backgroundImage':'url('+imgLi.bgSrc+')'}" :class="{ 'active': lottery.index == index} "> -->
    ...
    start () {
      ...
      this.lottery.timer = setInterval(() => {
        ...
        if (this.lottery.prizeStandardId) {
          if (this.$refs.pice[this.arrNum[this.lottery.index]].id == this.lottery.prizeStandardId) {
        ...
        if (this.timeFlag >= 10000 && !this.prizeName) { // 抽奖回调超时
          clearInterval(this.lottery.timer)
          this.timeFlag = 0
          Toast({
      ...
      }, Math.abs(100 * ((this.lottery.ring - 2) / 2)) == 0 ? 50 : Math.abs(200 * ((this.lottery.ring - 2) / 2)))
    },
    renderPrize () {
      ...
      this.$nextTick(() => {
        this.$refs.pice[4].style.backgroundImage = 'url(' + this.templateConfig.button + ')'
        // this.$refs.pice[4].style.backgroundImage = 'url(' + lotteryBtn + ')'
        this.$refs.pice[4].onclick = () => { // 绑定抽奖按钮事件
          if (this.chances > 0) { // 判断剩余抽奖次数
            if (this.clickFlag) {
