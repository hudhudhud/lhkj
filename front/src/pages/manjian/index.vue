  ...
  <div class="manjian">
    <div class="main">
      <div class="yule">
        <div
          class="yule-item"
          v-for="(std1,i) in act.productList[0].standardList"
          :key="std1+i"
          @click="createOrder(std1)"
        >
          <img
            :src="std1.standardActUrl"
            alt=""
          >
        </div>
      </div>
      <div class="shengyan">
        <div
          class="shengyan-item"
          v-for="(std2,i) in act.productList[1].standardList"
          :key="std2+i"
          @click="createOrder(std2)"
        >
          <img
            :src="std2.standardActUrl"
            alt=""
          > </div>
      </div>
      <div class="rule">
...
export default {
  data () {
    return {
      secondActId: process.env.PUBLISH_ENV == 'prod' ? 557221494 : 556523594,
      act: ''
    };
  },
  methods: {
    ...
    //     : null;
    //   var reg = new RegExp(std.accountPattern, 'ig');
