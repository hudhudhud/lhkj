<template>
  <div class="clientlogin">
    ...
    <div class="main-content">
      ...
      <p>如果您是上海高校在读学生，请填写学校名称，以便匹配相应活动：</p>
      <div class="login-form select-school">
        <input type="text" placeholder="请填写学校" v-model="schoolName">
      </div>
    </div>
    <div class="bottompic">
      <img src="./imgs/clientpicbg.png" alt>
    </div>
  </div>
</template>
...
export default {
  data () {
    return {
      customType: '',
      isShow: true, // true学生显示，false员工显示
      camous: [],
      campusId: '',
      schoolList: [],
      schoolId: '',
  ...
  methods: {
    async login () {
      ...
      let rs = await this.$post('/student/login', {
      ...
      if (rs.status == 200) {
        ...
        if (msg == '该手机号码已经注册为其它角色,请勿重复注册') {
          Toast('您是我行员工，请选择员工通道进入');
        } else {
          Toast(msg);
        }
      }
