<template>
  <div class="hemi-rotate">
    <div class="award">
      ...
      <div class="award-pointer" @click="start">
        <img :style="{width:pointerWidth}" :src="setting.pointerImage" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    rotate (prize) {
      ...
      this.warp.style.transform = `rotate(${angles}deg)`;
      this.warp.style.webkitTransform = `rotate(${angles}deg)`;
    ...
    async drawPannel () {
      ...
      for (let i = 0; i < this.setting.names.length; i++) {
        ...
        this.ctx.translate(
          r + Math.cos(angle + arc / 2) * textR + 15,
          r + Math.sin(angle + arc / 2) * textR + 15
        );
    ...
    redraw () {
      ...
      this.warp = this.$el.querySelector('.award-bg');
      let contentWidth = this.$el.querySelector('.award-bg').clientWidth;
      let borderWidth = contentWidth * 0.04;
      ...
      container.style.left = borderWidth * 1 + 'px';
