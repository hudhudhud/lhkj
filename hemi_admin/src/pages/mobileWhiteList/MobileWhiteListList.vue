<template>
  <LayoutPage>
    ...
    <LayoutTable ref="myTable" url="mobileWhiteList/query" :param="form " @loaded="returnLoaded" show-pager>
      ...
      <el-table-column label="操作" min-width="150">
        <template slot-scope="props">
          ...
          <!-- <el-button @click="edit(props.row)" v-if="props.row.public=='0' || admin=='1'" :loading="props.row.loading" plain>编辑</el-button>
          <el-button type="info" v-else :loading="props.row.loading" plain>公共模版不可编辑</el-button>
          <el-button @click="lookUp(props.row)" v-if="props.row.public=='0'|| admin=='1'" :loading="props.row.loading" plain>查看配置详情</el-button> -->
        </template>
      </el-table-column>
    </LayoutTable>
    ...
    <LayoutDialog title="Api限制配置" v-model="ApiShowDialog">
      <ApiLimitQueryConfigForm :ApiConfog="ApiConfog" @success="apiSuccess" @close="ApiShowDialog=false"></ApiLimitQueryConfigForm>
    </LayoutDialog>
  </LayoutPage>
</template>
...
import ApiLimitQueryConfigForm from "./ApiLimitQueryConfigForm"
export default {
  components: {
    EditListDetForm,
    ApiLimitQueryConfigForm
  },
  data () {
    return {
      listDef: {},
  ...
  methods: {
    returnLoaded (val1, val2) {
      this.admin = val2.payload.admin
      this.loading = false
    },
    edit (listDef) {
    ...
    async lookUp (prop) {
      if (prop.type == 1) {
        // 地区白名单
        this.$router.push({
          path: "mobileWhiteList/phoneLocationLimitConfigForm/" + prop.id
        })
      } else if (prop.type == 2) {
        // 号码白名单
        this.$router.push({
          path: "mobileWhiteList/BWMobileListList/" + 0 + "/" + prop.id
        })
      } else if (prop.type == 3) {
        // 号码黑名单
        this.$router.push({
          path: "mobileWhiteList/BWMobileListList/" + 1 + "/" + prop.id
        })
      } else if (prop.type == 4) {
        // API限制
        this.ApiConfog = prop
        this.ApiShowDialog = true
      }
    },
    onSearch () {
      this.loading = true
