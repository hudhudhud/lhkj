...
export default {
  ...
  methods: {
    async getMerchant () {
      var rs = await this.$post("merchant/getCustomMerchantByMercahntId", {id: this.$route.params.id})
      if (!rs.payload) {
        this.$message({
          type: "error",
    ...
    async getDetails () {
      var rs = await this.$post("merchant/getMerchantLimitDefDetaiuls", {id: this.$route.params.id})

      this.form.limitDefIdList = rs.payload.limitDefIdList
      this.form.id = rs.payload.id
    },
    async getListDef () {
      var rs = await this.$post("merchant/getListDefByMerchantIds")
