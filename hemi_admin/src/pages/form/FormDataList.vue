<template>
  <LayoutPage>
    <LayoutSearch>
      ...
      <el-form-item label="管辖行">
        <el-select v-model="searchForm.searchCriteria[0].value" @change="higherBankChange">
          ...
          <el-option v-for="(item, index) in higherBanks" :key="index" :label="item"
      ...
      <el-form-item label="网点">
        <el-select v-model="searchForm.searchCriteria[1].value">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="(item, index) in banks4Higher" :key="index" :label="item"
                     :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <LayoutTable ref="myTable" url="form/formData/16/12" :param="searchForm" @loaded="loading=false" show-pager>
      <el-table-column prop="submitDate" label="时间" width="100">
      </el-table-column>
      <el-table-column prop="data4" label="管辖行" width="100">
      </el-table-column>
      <el-table-column prop="data3" label="网点" width="180">
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      loadBanks: async function () {
        let rs = await this.$get("form/formResources/bankResources/CCB/higher?city=" + "苏州市")
        if (rs.status != "200") {
      ...
      higherBankChange: async function (higherBankAttr) {
        this.banks4Higher = this.allBank.filter(t => t.higherBank === higherBankAttr).flatMap(t => t.banks);
      ...
      exportExcel: async function () {
        ...
        let rs = await this.$post("/form/formData/16/12/export", excelForm)
        if (rs.status == "200") {
      ...
      saveRemark: function () {
        let rowsData;
        rowsData = JSON.parse(JSON.stringify(this.$refs.myTable.list))
          .map(t => {
            t.originData = null;
            t.remarks = t.remark
            return t;
        ...
        this
          .$post("/form/formData/remark", rowsData)
          .then(resp => {
            if (resp.status == "200") {
