  ...
  <LayoutPage>
    <LayoutSearch>
      <el-form-item label="时间">
        <el-date-picker v-model="searchForm.dateGap" value-format="yyyy-MM-dd HH:mm:ss" type="datetimerange" align="right"
      ...
      <el-form-item label="意向品牌">
        <el-select v-model="searchForm.searchCriteria[2].value">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="(item, index) in carBrandArr" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="意向车价">
        <el-select v-model="searchForm.searchCriteria[3].value">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="(item, index) in intentionalCarPriceArr" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <LayoutTable ref="myTable" url="form/formData/15/12" :param="searchForm" @loaded="loading=false" show-pager>
      <el-table-column prop="submitDate" label="时间" width="100">
      </el-table-column>
      <el-table-column prop="data6" label="管辖行" width="150">
      </el-table-column>
      ...
      <el-table-column prop="data3" label="意向品牌" width="120">
      </el-table-column>
      <el-table-column prop="data4" label="意向车价" width="100">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    loadResources: function() {
      let $this = this;
      this.$get("/form/formResources/values/" + "carBrand")
        .then(success => {
          if (success.status != "200") {
            $this.$message.error(success.error);
          } else {
            $this.carBrandArr = success.payload;
          }
        })
        .catch(error => this.$message.error(error));
      this.$get("/form/formResources/values/" + "intentionalCarPrice")
        .then(success => {
          if (success.status != "200") {
            $this.$message.error(success.error);
          } else {
            $this.intentionalCarPriceArr = success.payload;
          }
        })
        .catch(error => this.$message.error(error));
    },
    ...
    exportExcel: async function() {
      ...
      let rs = await this.$post("/form/formData/15/12/export", excelForm);
      if (rs.status == "200") {
