...
export default {
  ...
  methods: {
    ...
    async loadTimeConfig () {
      this.submiting = true
      let rs = await this.$post("activity/get_act_time_config", {actId: this.form.actId})
      if (rs.status == "200") {
    ...
    async doSubmit () {
      ...
      this.form.timeIntervalStr = this.form.timeInterval.map(item => {
        return item.startTime + "-" + item.endTime
      }).join(",")
      this.form.dateTime = this.form.dateTime.sort((a, b) => { return a - b })
      this.form.dateTimeStr = this.form.dateTime.join(",")
      console.log(this.form.timeIntervalStr)

      let rs = await this.$post("activity/save_act_time_config", this.form)
