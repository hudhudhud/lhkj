<template>
  <LayoutPage>
    <LayoutSearch>
      ...
      <el-form-item label="期望价格">
        <el-select v-model="searchForm.searchCriteria[3].value">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="(item, index) in intentionalHousePriceArr" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <LayoutTable ref="myTable" url="form/formData/13/12" :param="searchForm" @loaded="loading=false" show-pager>
      <el-table-column prop="submitDate" label="时间" width="100">
      </el-table-column>
      <el-table-column prop="data7" label="管辖行" width="100">
      </el-table-column>
      <el-table-column prop="data6" label="网点" width="180">
      </el-table-column>
      <el-table-column prop="data1" label="客户姓名" width="100">
      </el-table-column>
      <el-table-column prop="data2" label="联系方式" width="100">
      </el-table-column>
      <el-table-column prop="data3" label="期望小区" width="120">
      </el-table-column>
      <el-table-column prop="data4" label="期望面积" width="100">
      </el-table-column>
      <el-table-column prop="data5" label="期望价格" width="100">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    loadResources: function() {
      ...
      this.$get("/form/formResources/values/" + "intentionalHousePrice")
        .then(success => {
          if (success.status != "200") {
            $this.$message.error(success.error);
          } else {
            $this.intentionalHousePriceArr = success.payload;
          }
        })
        .catch(error => this.$message.error(error));
    },
    ...
    exportExcel: async function() {
      ...
      let rs = await this.$post("/form/formData/13/12/export", excelForm);
      if (rs.status == "200") {
